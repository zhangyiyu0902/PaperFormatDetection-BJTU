...
namespace PaperFormatDetection.Paperbase
{
    class Catalog
    {
        ...
        public void calatogOrderDetect(Paragraph p, WordprocessingDocument doc)
        {
            ...
            if (str.IndexOf("．") >= 0)
            {
                Util.printError("目录中的点号 . 应为英文(占一个字符)" + "  ----" + str);
                str = str.Replace("．", ".");
            }

            string order = "";
            MatchCollection mc = Regex.Matches(str, "^[0-9一二三四五六七八九十]+([.][0-9一二三四五六七八九十]+)*");
            if (mc.Count > 0)
            {
                foreach (Match m in mc)
                {
                    ...
                    {
                        if (!Util.IsInt(str.Substring(0, 1)))
                        {
                            Util.printError("目录序号错误，应为阿拉伯数字 " + one_level + "  ----" + str);
                        }
                        else if (order != one_level)
                        {
